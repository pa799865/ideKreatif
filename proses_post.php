...
if (isset($_POST['simpan'])) {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $imagePath)) {
        $query = "INSERT INTO posts (post_title, content, created_at, category_id, user_id, image_path) VALUES ('$postTitle', '$content', NOW(), '$categoryId', '$userId', '$imageName')";
        ...
        if($conn->query($query) === TRUE) {
            ...
            $_SESSION['notification'] = [
                'type' => 'danger',
                'message' => 'Erorr adding post: ' . $conn->error
            ];
        }
    } else {
        $_SESSION['notification'] = [
